...
public class DepartmentDaoImpl implements DepartmentDao {
    ...
    public boolean updateDepartment(Department dept) {
        try (Connection con = DbUtil.getConnection();
            ...
            if (rowsAffected == 0) {
                throw new DepartmentNotFoundException("No department found to update with ID " + dept.getDeptId());
            }
    ...
    public boolean deleteDepartment(int id) throws DepartmentNotFoundException {
        try (Connection con = DbUtil.getConnection();
            ...
            if (rows == 0) {
                throw new DepartmentNotFoundException("No department found to delete with ID " + id);
            }
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
